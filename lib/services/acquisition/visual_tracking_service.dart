...
class VisualTrackingService {
  ...
  VisualTrackingService({
    this.featureTracker = const VisualFeatureTracker(),
    this.motionEstimator = const VisualMotionEstimator(),
  });
  ...
  Future<VisualTrackingResult> process(OpticalFrame frame) async {
    if (!_running) {
      return VisualTrackingResult.unavailable(frame, 'VISUAL_TRACKING_NOT_STARTED');
    }

    final file = File(frame.path);
    if (!await file.exists()) {
      return VisualTrackingResult.unavailable(frame, 'CAMERA_FRAME_NOT_FOUND');
    }

    final bytes = await file.readAsBytes();
    return processBytes(frame, bytes);
  }
  ...
  VisualTrackingResult processBytes(OpticalFrame frame, Uint8List bytes) {
    ...
    if (previous == null) {
      return VisualTrackingResult(
        ...
        matches: const [],
        motion: null,
        status: VisualTrackingStatus.initialized,
        reason: 'FIRST_FRAME_INITIALIZED',
      );
    }
    ...
    if (!motion.valid) {
      return VisualTrackingResult(
        ...
        matches: matches,
        motion: motion,
        status: VisualTrackingStatus.insufficientData,
        reason: 'INSUFFICIENT_TRACKING_QUALITY',
      );
    }
    ...
    return VisualTrackingResult(
      ...
      matches: matches,
      motion: motion,
      status: VisualTrackingStatus.valid,
      reason: 'REAL_FRAME_TO_FRAME_MOTION',
    );
  }
}
...
class VisualTrackingResult {
  ...
  factory VisualTrackingResult.unavailable(OpticalFrame frame, String reason) =>
